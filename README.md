...
chunking (до 1 800 символов, overlap 250)
          │
          ▼
точные токены → BM25-подобный retrieval → coverage gate → top-k
          │                                      │
          │ нет релевантного контекста             │ есть контекст
          ▼                                      ▼
...
UI принимает один PDF до 50 МБ, обрабатывает его локально и хранит историю вопросов текущей сессии. Модель можно изменить в разделе «Настройки модели».
